...
module.exports = function getEntriesForUser(uuid, callback) {
    ...
    sql.query('SELECT * FROM JDEntries LEFT JOIN JDCars ON JDEntries.entCarId = JDCars.carId LEFT JOIN JDRoadCondition ON JDEntries.entRoaId = JDRoadCondition.roaId LEFT JOIN JDCompanions ON JDEntries.entComId = JDCompanions.comId WHERE entUseId = ?', [uuid], (err, rows) => {
