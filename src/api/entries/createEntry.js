...
module.exports = function createEntry(uuid, date, startMileage, endMileage, routeDest, notes, carId, roadConditionId, companionId, callback) {

    const entId = uuidv4();
    const entDate = new Date(date * 1000).toISOString().slice(0, 19).replace('T', ' ');

    sql.query('INSERT INTO JDEntries (entId, entDate, entStartMileage, entEndMileage, entRouteDest, entNotes, entCarId, entRoaId, entComId, entUseId) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [entId, entDate, startMileage, endMileage, routeDest, notes, carId, roadConditionId, companionId, uuid], (err) => {
